...
Describe 'Measure-VmHappiness' {
    ...
    Describe 'CPU Happiness' {
        ...
        It 'returns 100 when host CPU is well below the 70% stress threshold' {
            ...
            $host = New-HostMetrics -Name 'N1' -CpuUtil 50.0
            $vm   = New-VmMetrics  -Name 'VM1' -CpuUtil 80.0
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).CpuHappiness | Should -Be 100.0
        }

        It 'returns 100 when host CPU is exactly at the stress threshold (70%)' {
            # stress = 0  →  100 regardless of vm demand
            $host = New-HostMetrics -Name 'N1' -CpuUtil 70.0
        ...
        It 'degrades linearly when host CPU is 85% and VM demand is 80%' {
            ...
            $host = New-HostMetrics -Name 'N1' -CpuUtil 85.0
            $vm   = New-VmMetrics  -Name 'VM1' -CpuUtil 80.0
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).CpuHappiness | Should -Be 60.0
        }

        It 'returns 50 when host CPU is 100% and VM demand is 50%' {
            # stress = 1.0  →  100 - 1.0*50 = 50
            $host = New-HostMetrics -Name 'N1' -CpuUtil 100.0
            $vm   = New-VmMetrics  -Name 'VM1' -CpuUtil 50.0
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).CpuHappiness | Should -Be 50.0
        }
        ...
        It 'returns 0 when host CPU is 100% and VM is fully demanding' {
            ...
            $host = New-HostMetrics -Name 'N1' -CpuUtil 100.0
            $vm   = New-VmMetrics  -Name 'VM1' -CpuUtil 100.0
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).CpuHappiness | Should -Be 0.0
        }
    ...
    Describe 'Memory Happiness — Dynamic Memory' {
        ...
        It 'returns 100 when pressure indicates surplus (< 100)' {
            $host = New-HostMetrics -Name 'N1' -CpuUtil 20.0
            $vm   = New-VmMetrics  -Name 'VM1' -DynMem $true -Pressure 80.0
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).MemHappiness | Should -Be 100.0
        }
        ...
        It 'returns 50 at the midpoint of the degradation range (pressure 125)' {
            ...
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).MemHappiness | Should -Be 50.0
        }
        ...
        It 'returns 0 at the upper degradation boundary (pressure 150)' {
            ...
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).MemHappiness | Should -Be 0.0
        }
    ...
    Describe 'Memory Happiness — Static Memory' {
        ...
        It 'returns 100 when host memory utilization is low (40%)' {
            ...
            $host = New-HostMetrics -Name 'N1' -CpuUtil 20.0 -TotalMemMB 100000 -AvailMemMB 60000
        ...
        It 'returns 100 at exactly 70% host memory utilization' {
            ...
            $host = New-HostMetrics -Name 'N1' -CpuUtil 20.0 -TotalMemMB 100000 -AvailMemMB 30000
            $vm   = New-VmMetrics  -Name 'VM1' -DynMem $false
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).MemHappiness | Should -Be 100.0
        }
        ...
        It 'returns 50 at 80% host memory utilization' {
            ...
            $host = New-HostMetrics -Name 'N1' -CpuUtil 20.0 -TotalMemMB 100000 -AvailMemMB 20000
            $vm   = New-VmMetrics  -Name 'VM1' -DynMem $false
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).MemHappiness | Should -Be 50.0
        }
        ...
        It 'returns 0 at exactly 90% host memory utilization' {
            ...
            $host = New-HostMetrics -Name 'N1' -CpuUtil 20.0 -TotalMemMB 100000 -AvailMemMB 10000
            $vm   = New-VmMetrics  -Name 'VM1' -DynMem $false
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).MemHappiness | Should -Be 0.0
        }
        ...
        It 'returns 0 above 90% host memory utilization' {
            ...
            $host = New-HostMetrics -Name 'N1' -CpuUtil 20.0 -TotalMemMB 100000 -AvailMemMB 5000
            $vm   = New-VmMetrics  -Name 'VM1' -DynMem $false
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).MemHappiness | Should -Be 0.0
        }
    }
    ...
    Describe 'Combined Score' {
        ...
        It 'returns 100 when both CPU and memory are fully satisfied' {
            ...
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).HappinessScore | Should -Be 100.0
        }
        ...
        It 'averages CPU and memory happiness with equal weights' {
            ...
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).HappinessScore | Should -Be 55.0
        }
        ...
        It 'applies custom weights correctly' {
            ...
            $host = New-HostMetrics -Name 'N1' -CpuUtil 85.0
            $vm   = New-VmMetrics  -Name 'VM1' -CpuUtil 80.0 -DynMem $true -Pressure 125.0
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host -CpuWeight 0.3 -MemoryWeight 0.7).HappinessScore |
                Should -Be 53.0
        }
        ...
        It 'honours CPU-only weighting' {
            ...
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host -CpuWeight 1.0 -MemoryWeight 0.0).HappinessScore |
                Should -Be 50.0
        }
        ...
        It 'honours memory-only weighting' {
            ...
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host -CpuWeight 0.0 -MemoryWeight 1.0).HappinessScore |
                Should -Be 50.0
        }
        ...
        It 'returns 0 when both CPU and memory are fully unhappy' {
            ...
            (Measure-VmHappiness -VmMetrics $vm -HostMetrics $host).HappinessScore | Should -Be 0.0
        }
    }
