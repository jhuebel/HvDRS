...
| File | Written by | Read by | Missing | Corrupt |
|---|---|---|---|---|
| `rules.json` | `Add/Set/Remove-HvDRSAffinityRule` | `Get-AffinityRuleSet` (all consumers) | Empty rule set — nothing configured yet | **Throws.** A parse failure must never be silently treated as "no rules": that would let `Invoke-HvDRS` plan migrations that break enforced rules, and let the next `Add/Set/Remove-HvDRSAffinityRule` resave the file with only its own change, permanently wiping every other cluster's rules |
| `groups.json` | `Add/Set/Remove-HvDRSGroup` | `Get-AffinityRuleSet` (group expansion), `Get-HvDRSGroup` | No groups defined; rules behave as literal-only | **Throws**, and propagates out of `Get-AffinityRuleSet` — same reasoning: a rule that depends on a group would silently lose those members |
| `automation-overrides.json` | `Set/Remove-HvDRSVMAutomationLevel` | `Invoke-HvDRS`/`Invoke-HvStorageDRS` (execution gating), `Get-HvDRSVMAutomationLevel` | No overrides; every VM is FullyAutomated | **Throws** — treating it as empty would silently un-pin every VM set to Manual and let the next pass migrate it |
| `maintenance.lock` | `Enable/Disable-HvDRSMaintenance` | `Invoke-HvDRS`/`Invoke-HvStorageDRS` (checked via `Test-Path`, not JSON-parsed) | Not in maintenance — this is the normal state, not a failure | N/A — content is a free-text reason string, never parsed |
| `history\<ClusterName>.json` | `Merge-HvDRSTrendSnapshot` | Same function, next pass | Fresh single-entry window (current snapshot only) | Fresh single-entry window, with a `Write-Warning` — a bad trend file only degrades smoothing, it doesn't skip rule/pin enforcement, so fail-soft is safe here |

The three rule/group/override stores gate HVDRS's hard-rule and Manual-pin safety guarantees, so their loaders fail **closed**: `Test-Path` → if absent, return an empty result (`,@()`) — that's the legitimate "nothing configured yet" state; otherwise `Get-Content -Raw | ConvertFrom-Json`, and if that fails or the expected top-level property (`Rules`/`Groups`/`Overrides`) is missing, **throw** rather than degrade to empty. This deliberately breaks the calling `Invoke-HvDRS`/`Invoke-HvStorageDRS` pass (and any rule/group/override CRUD call) rather than silently running with rules or pins disabled, and it stops a rule-mutating cmdlet from resaving over — and thereby destroying — a corrupt store it couldn't actually parse. `history\<ClusterName>.json` is the one exception: it only affects trend smoothing, not enforcement, so it keeps the original fail-soft behavior (fresh window, `Write-Warning`).
...
1. **Compliance pass** — every *enforced* rule currently violated gets a proactive fix: the best (VM, destination) pair that resolves it without introducing a new hard violation, evaluated ahead of and independent of any happiness threshold. Compliance migrations are always included in the plan. A rule spanning 3+ VMs (`VmVmAffinity`/`VmVmAntiAffinity`, and their storage equivalents `VmVmCsvAffinity`/`VmVmCsvAntiAffinity`) can need more than one move to fully satisfy — e.g. three VMs sharing one host under a hard anti-affinity rule can't all be separated by moving just one of them — so this pass computes a numeric **severity** for each enforced rule (`$ruleSeverity`/`$csvRuleSeverity`: excess distinct hosts/CSVs for an affinity rule, "doubled-up" members for an anti-affinity rule, violating-member count for the single-VM host/CSV rule types) and iterates, each round taking whichever move reduces some violated rule's severity the most, until every enforced rule is satisfied or no move helps at all. A move that only partially resolves a rule is accepted and recommended just like a fully-resolving one — `ComplianceReason` says which.
...
The two happiness passes differ in one respect worth calling out: `Find-StorageMigrationCandidates` keeps proposing moves off the **same** unhappy source CSV (re-scoring it against the simulated state after each one) until it clears the threshold or no VM on it can be beneficially placed anywhere, since one VM rarely brings a large CSV's space score up on its own. `Find-MigrationCandidates` only ever proposes one move per VM per pass, which is the right granularity on the compute side (there's no equivalent "per-node happiness" to re-check — a node isn't scored, only the VMs on it are). `Find-StorageMigrationCandidates`'s happiness pass also refuses any candidate destination whose own projected score would land below the aggression threshold, whether it was already unhappy or the move alone would make it so — it must not fix one CSV by breaking another. (Pass 1's compliance fixes are exempt from that check, matching the compute side: an enforced rule violation is fixed regardless of happiness cost.)
